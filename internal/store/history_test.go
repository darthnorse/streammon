...
func TestGetItemLastWatchedAt(t *testing.T) {
	s := newTestStoreWithMigrations(t)
	serverID := seedServer(t, s)
	...
	// No history at all — should return false
	ts, found, err := s.GetItemLastWatchedAt(ctx, serverID, "movie1")
	if err != nil {
		t.Fatalf("GetItemLastWatchedAt: %v", err)
	}
	if found {
		t.Errorf("expected found=false for unwatched item, got true (ts=%v)", ts)
	}

	// Add a watch entry for a movie (matched by item_id)
	watchTime := now.Add(-48 * time.Hour)
	if err := s.InsertHistory(&models.WatchHistoryEntry{
		ServerID: serverID, UserName: "alice", MediaType: models.MediaTypeMovie,
		ItemID: "movie1", Title: "Test Movie",
		StartedAt: watchTime.Add(-time.Hour), StoppedAt: watchTime,
	}); err != nil {
		t.Fatal(err)
	}
	...
	if !found {
		t.Fatal("expected found=true after inserting history")
	}
	if ts.Sub(watchTime).Abs() > time.Second {
		t.Errorf("expected timestamp ~%v, got %v", watchTime, ts)
	}

	// Add a newer watch — should return the later timestamp
	laterWatch := now.Add(-24 * time.Hour)
	if err := s.InsertHistory(&models.WatchHistoryEntry{
		ServerID: serverID, UserName: "bob", MediaType: models.MediaTypeMovie,
		ItemID: "movie1", Title: "Test Movie",
		StartedAt: laterWatch.Add(-time.Hour), StoppedAt: laterWatch,
	}); err != nil {
		t.Fatal(err)
	}
	...
	if ts.Sub(laterWatch).Abs() > time.Second {
		t.Errorf("expected timestamp ~%v, got %v", laterWatch, ts)
	}

	// TV show matched by grandparent_item_id
	tvWatch := now.Add(-12 * time.Hour)
	if err := s.InsertHistory(&models.WatchHistoryEntry{
		ServerID: serverID, UserName: "alice", MediaType: models.MediaTypeTV,
		ItemID: "ep1", GrandparentItemID: "show1", Title: "Episode 1",
		StartedAt: tvWatch.Add(-30 * time.Minute), StoppedAt: tvWatch,
	}); err != nil {
		t.Fatal(err)
	}

	ts, found, err = s.GetItemLastWatchedAt(ctx, serverID, "show1")
	if err != nil {
		t.Fatalf("GetItemLastWatchedAt (TV): %v", err)
	}
	if !found {
		t.Fatal("expected found=true for TV show")
	}
	if ts.Sub(tvWatch).Abs() > time.Second {
		t.Errorf("expected timestamp ~%v, got %v", tvWatch, ts)
	}

	// Different server — should not find it
	_, found, err = s.GetItemLastWatchedAt(ctx, serverID+999, "movie1")
	if err != nil {
		t.Fatalf("GetItemLastWatchedAt (wrong server): %v", err)
	}
	if found {
		t.Error("expected found=false for different server")
	}
}
