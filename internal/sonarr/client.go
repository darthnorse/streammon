...
type seasonPassMonitor struct {
	Monitor string `json:"monitor"`
}

type seasonPassRequest struct {
	Series            []seasonPassSeries `json:"series"`
	MonitoringOptions seasonPassMonitor  `json:"monitoringOptions"`
}
...
func (c *Client) SetMonitorFuture(ctx context.Context, seriesID int) error {
	body := seasonPassRequest{
		Series:            []seasonPassSeries{{ID: seriesID}},
		MonitoringOptions: seasonPassMonitor{Monitor: "future"},
	}
	data, err := json.Marshal(body)
	if err != nil {
		return fmt.Errorf("marshal seasonpass: %w", err)
	}
	_, err = c.DoPost(ctx, "/seasonpass", data)
	return err
}
